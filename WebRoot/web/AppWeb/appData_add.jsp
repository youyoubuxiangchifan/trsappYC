  ...
  <div class="fr B mb10 i_Gr">
    ...
    <form name="formData" id="formData" action="appWeb.do?method=saveAppData" method="post" enctype="multipart/form-data" onsubmit="setCboxValue();">
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="i_Gr_7 mb10">
       ...
       <c:forEach items="${requestScope.fieldList}" var="field" varStatus="fstep">
	...
	<c:choose>
       	 <c:when test="${fn:length(requestScope.isGrpFields) > 0}">
       	 	<%@ include file="./appData_add_grp.jsp"%> 
       	 </c:when>
       	 <c:otherwise>
       	 <tr>
             	<c:choose>
     ...
     <c:if test="${appInfo.isSupAppendix eq 1}">
        ...
        <tr>
             ...
             <td width="79%">
             <input type="text" id="checkCode" name="checkCode" class="{required:true}"/>
...
<script type="text/javascript">
	...
	function js_changetab(obj){
		$(".js_tab").hide();
		$("#"+obj).show();
		var groupName = "${isGrpFields[0].enmValue}";
		groupName = groupName.split("~");
		for (i=0;i<groupName.length ;i++){
			$("#"+groupName[i]+" input").attr("disabled","disabled");
			$("#"+groupName[i]+" select").attr("disabled","disabled");
			$("#"+groupName[i]+" textarea").attr("disabled","disabled");
		}
		$("#"+obj+" input").removeAttr("disabled");
		$("#"+obj+" select").removeAttr("disabled");
		$("#"+obj+" textarea").removeAttr("disabled");
	}
</script>
</body>
</html>
