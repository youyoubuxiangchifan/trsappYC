...
<div class="pageHeader required-validate" style="border:1px #B8D0D6 solid">
	<form id="pagerForm" onsubmit="return navTabSearch(this);" action="sysconfig.do?method=listOper" method="post">
	...
	<div class="searchBar">
			...
			<select name="selectFiled">   						
   						<option value="opname" <c:if test="${selectFiled == 'opname'}">selected="selected"</c:if>>操作名称</option>
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="sysconfig.do?method=editOper" target="dialog" mask="true" width="510" height="300"><span>添加</span></a></li>
			<li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="operIDs" postType="string" href="${basePath}sysconfig.do?method=delOper" class="delete"><span>删除</span></a></li>
		</ul>
	</div>
	<table class="table" width="100%" layoutH="110">
		<thead>
			<tr>
				<th width="50"><input type="checkbox" class="checkboxCtrl" group="operIDs" value="0"/>全选</th>
				<th width="120" align="center">操作名称</th>
				<th>操作描述</th>
				<th width="80" align="center">操作标识</th>
		...
		<c:forEach items="${requestScope.operList}" var="oper" varStatus="num">
			<tr target="sid_oper" rel="${oper.operid }">
				<td><input type="checkbox" name="operIDs" value="${oper.operid }"/></td>
				<td>${oper.opname }</td>
				<td>${oper.opdesc }</td>
				<td>${oper.operFlag }</td>
				<td>${oper.crData }</td>
				<td>${oper.cruser }</td>
				<td>
				<a href="sysconfig.do?method=editOper&operID=${oper.operid }" target="dialog" 
