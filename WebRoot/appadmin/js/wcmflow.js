function GooFlow(bgDiv,property){
	...
	else if(navigator.userAgent.indexOf("MSIE 9.0")>0 && document.documentMode<9){
		GooFlow.prototype.useSVG="";
	...
	//alert(GooFlow.prototype.useSVG+navigator.userAgent+document.documentMode);
//初始化区域图的对象
	...
	this.$nodeDom={};
	this.$areaDom={};
	this.$delNodeData={};//add by liuzhuan
	this.$delLineData={};//add by liuzhuan
	this.$altFlag=false;
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		e.data.inthis.$max++;
		e.data.inthis.$altFlag = true;//更新标识 add by liuzhuan
	  });
	  //划线时用的绑定
	  ...
	  this.$workArea.mouseup({inthis:this},function(e){
		...
		e.data.inthis.$altFlag = true;//更新标识add by liuzhuan
	  });
	  ...
	  this.$lineMove.on("mousedown",{inthis:this},function(e){
		  ...
		  //This.$altFlag = true;//更新标识add by liuzhuan
	  });
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		$(this.$draw).delegate(tmpClk,"dblclick",{inthis:this},function(e){
			...
			var dwzLeft = $("div.nav-left").width() + 6;//dwz左侧宽  liujian
			var dwzTop = $("div.s-header").height() + $("div.zd-nav").height() + 33;//dwz头部高  liujian
			This.$textArea.val(oldTxt).css({display:"block",width:120,height:14,
				left:t.left+x-This.$workArea[0].parentNode.scrollLeft-dwzLeft, //liujian
				top:t.top+y-This.$workArea[0].parentNode.scrollTop-dwzTop}).data("id",This.$focus).focus();//liujian
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".ico","mousedown",{inthis:this},function(e){
			...
			This.$ghost.css({display:"block",
				...
				top:This.$nodeData[id].top+t.top-This.$workArea[0].parentNode.scrollTop+hack-dwzTop+"px",//liujian
				left:This.$nodeData[id].left+t.left-This.$workArea[0].parentNode.scrollLeft+hack-dwzLeft+"px",cursor:"move" //liujian
			});
			...
			document.onmousemove=function(e){
				...
				This.$ghost.css({left:X+hack-dwzLeft+"px",top:Y+hack-dwzTop+"px"}); //liujian 拖动是阴影位置适应DWZ框架
				isMove=true;
			}
			document.onmouseup=function(e){
		...
		this.$workArea.delegate(".ico + td","dblclick",{inthis:this},function(e){
			//wenjunhui
			//alert($(this).parents(".GooFlow_item").attr("id"));
			var This=e.data.inthis;
			var ids = $(this).parents(".GooFlow_item").attr("id");
			_addNodes(This,ids);
			This.$altFlag = true;//更新标识add by liuzhuan
			//var oldTxt=this.innerHTML;
			//var This=e.data.inthis;
		...
		this.$workArea.delegate(".GooFlow_item > div > div[class!=rs_close]","mousedown",{inthis:this},function(e){
			...
			This.$ghost.css({display:"block",
				width:This.$nodeData[id].width-2+"px", height:This.$nodeData[id].height-2+"px",
				top:This.$nodeData[id].top+t.top-This.$workArea[0].parentNode.scrollTop+hack-dwzTop+"px",//liujian
				left:This.$nodeData[id].left+t.left-This.$workArea[0].parentNode.scrollLeft+hack-dwzLeft+"px",cursor:cursor //liujian
			});
			var X,Y;
			X=ev.x-t.left+This.$workArea[0].parentNode.scrollLeft;
			Y=ev.y-t.top+This.$workArea[0].parentNode.scrollTop;
	...
	delNode:function(id){
		...
		this.$delNodeData[id]=this.$nodeData[id];//add by liuzhuan
		...
		for(var k in this.$lineData){
			if(this.$lineData[k].from==id||this.$lineData[k].to==id){
				this.$delLineData[k] = this.$lineData[k];//add by liuzhuan
	...
	exportData:function(){
		return {title:this.$title,initNum:this.$max,nodes:this.$nodeData,lines:this.$lineData,areas:this.$areaData};
	},
	//清空工作区及已载入的数据
	clearData:function(){
	...
	destrory:function(){
		...
		this.$areaCount=0;
		this.$areaCount=0;
		this.$delLineData=null;//add by liuzhuan
		this.$delNodeData=null;//add by liuzhuan
		this.$altFlag=false;//add by liuzhuan
	},
///////////以下为有关画线的方法
	//绘制一条箭头线，并返回线的DOM
	drawLine:function(id,sp,ep,mark,dash){
		...
		if(GooFlow.prototype.useSVG!=""){
			...
			path.setAttribute("stroke-width",2.0);//1.4 update by liuzhuan
	...
	addLine:function(id,json){
		...
		if(this.$lineData[id].type=="sl")
			this.$lineDom[id]=GooFlow.prototype.drawLine(id,res.start,res.end,json.mark, true);//update by liuzhuan
		else
	...
	resetLines:function(id,node){
		for(var i in this.$lineData){
		  ...
		  if(this.$lineData[i].type=="sl"){
		  	this.$lineDom[i]=GooFlow.prototype.drawLine(i,res.start,res.end,this.$lineData[i].marked, true);//update by liuzhuan
		  }
		  else{
	...
	setLineType:function(id,newType){
		...
		//如果是变回直线
		  ...
		  this.$lineDom[id]=GooFlow.prototype.drawLine(id,res.start,res.end,this.$lineData[id].marked||this.$focus==id, true);//update by liuzhuan
		  this.$draw.appendChild(this.$lineDom[id]);
		  if(GooFlow.prototype.useSVG==""){
	...
	delLine:function(id){
		...
		this.$delLineData[id]=this.$lineData[id];//add by liuzhuan
		this.$draw.removeChild(this.$lineDom[id]);
