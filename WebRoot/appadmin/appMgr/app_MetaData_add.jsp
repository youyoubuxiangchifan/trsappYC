							...
							<!-- 简单编辑器开始 -->
								...
								<c:if test="${fields.fieldType eq 'text' && fields.inDetail eq 1}">
									<c:set var="tab_count" value="${tab_count+1}"></c:set>
										...
										<td width="40%" align="left">
											<c:choose>
												<c:when test="${fields.fieldName eq 'CRUSER'}">
								...
								<c:if test="${fields.fieldType eq 'hidden' && fields.inDetail eq 1}">
									<p class="_p">
										...
										<input type="text" name="${fields.fieldName }" size="50"/>
									</p>	
								</c:if>	
						</c:if>
					</c:forEach>
...
<script>
	$('.xheditor').each(function(){
		...
		$(this).xheditor({tools:editorTools,skin:'vista',upLinkUrl:'appUpload.do?method=doXheditorUpload',upLinkExt:'zip,rar,txt,doc,docx,xls,xlsx',upImgUrl:'appUpload.do?method=doXheditorUpload',upImgExt:'jpg,jpeg,gif,png',upFlashUrl:'appUpload.do?method=doXheditorUpload',upFlashExt:'swf',upMediaUrl:'appUpload.do?method=doXheditorUpload',upMediaExt:'wmv,avi,wma,mp3,mid',cleanPaste:0});
	});
	$(function(){
