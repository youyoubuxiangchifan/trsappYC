 ...
 * @author liu.zhuan
 * @ClassName: AppDataPushToWcmJob
 * @Copyright: Copyright (c) TRS北京拓尔思信息技术股份有限公司<BR>
 * @Company: TRS北京拓尔思信息技术股份有限公司杭州办事处(www.trs.com.cn)<BR>
 * @date 2014-10-22 下午02:02:49
 * @version 1.0
 */
@Service
public class AppDataPushToWcmJob {
	private static  Logger LOG =  Logger.getLogger(AppDataPushToWcmJob.class);
	@Autowired
		...
		try {
			...
			for (Object object : fromApps) {
				...
				metaDataList = publicAppBaseService.queryAppInfos(appInfo.getMainTableName(), sSelectField.toString(), 
						"deleted = 0 and APPID = ? and syncflag != 1", parameters, "crtime asc", 10000);
				if(metaDataList == null || metaDataList.size() == 0){
					LOG.warn("应用没有有效数据，应用编号-[" + appInfo.getAppId() + "],推送任务将跳过此应用！");
					continue;
				}
